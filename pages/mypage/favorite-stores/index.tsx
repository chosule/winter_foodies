...
const FavoriteStoresPage = (props) => {
    const { data: stores, isSuccess } = useQuery(
        ["heartStorePosts"], getHeartStore , {initialData: props.heartStorePosts}
    );
  return (
    <>
     <HeaderLayout headerTitle="찜한매장"/>
      {isSuccess ? (<>
        {stores?.data?.map(({pictureUrl,address,distance,rating,storeName,id}:FavoriteStoreType) =>(
          <StyledBox key={id} width="100%" height="74px" backgroundcolor="#fff">
            <StyledImage src={String(pictureUrl)} alt="사진" width={73} height={73}/>
            <MyPageUI.Flex alignItems="center" justifyContent="space-between" width="100%" padding="10px">
              <div>
                ...
                <MyPageUI.Flex gap="15px" alignItems="center" lineHeight="1.8">
                  ...
                  <MyPageUI.Text fontSize="11px" color="#353535">{useConverterMeter(Number(distance))}m</MyPageUI.Text>
                </MyPageUI.Flex>
              </div>
